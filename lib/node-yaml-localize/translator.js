...
var translator = function translator() {
	 ...
	 */
	this.setPath = function (path, callback) {
		if (path != _path) {
			_initialized = false;
			_path = path;
	...
	/**
	 * Sets the path of YAML files synchronously
	 */	
	this.setPathSync = function(path) {
		if (path != _path) {
			_initialized = false;
			_path = path;
			_initializeSync();
	...
	var _initialize = function(callback) {
		var files = _walk(_path, function(file,results) {
			...
			if (results) {
				...
				_initialized = true;
				//console.log("yaml-localize ready, loaded " + results.length + " files"); //This hurts console apps 
				return callback();
			} else {
				return callback();
			}
	...
	/**
	 * Initialize translator by parsing all YAML files synchronously
	 */
	var _initializeSync = function () {
		var files = _walk(_path, function(file,results) {
